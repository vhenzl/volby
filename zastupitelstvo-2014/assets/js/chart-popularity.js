(function () {
    ...
    function createChart(elementId, h, data, orderFn) {
        ...
        var yValueMin = d3.min(data, yValueSelector) * yTopSpace;
        var yPercentageMin = d3.min(data, yPercentageSelector) * yTopSpace;

        xScale.domain(data.map(xSelector));
        yScale.domain([yPercentageMin, yPercentageMax]);
        ...
        label.transition()
            .duration(500)
            .delay(function (d, i) { return i * 10; })
            .attr('y', function (d, e) { return yCalculator(d, e) - 5; });
    ...
    var totalVotes = _.reduce(raw, function (memo, v) { return memo + v.Votes; }, 0);
    var average = totalVotes / raw.length;

    var sorted1 = _.sortBy(raw, function (v) { return -1*v.Votes; });
    var data1 = _.map(sorted1, function (v, i) {
        var candidateList = candidateLists['2014'][v.ListNumber.toString()];
        return {
            Number: v.ListNumber + '.' + v.Number,
            Age: v.Age,
            Votes: v.Votes,
            Percent: ((v.Votes - average)/average),
            Mandate: v.Mandate,
            Name: v.Name,
